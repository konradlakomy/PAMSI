...
template <class T>
long AssocArray<T>::Size()
{
	return array.size();
}

template <class T>
bool AssocArray<T>::IsEmpty()
{
	return array.empty();
}

template <class T>
bool AssocArray<T>::Print() 
{
	if(!IsEmpty())
	{
		...
		for(it=array.begin(); it!=array.end(); ++it) 
			cout << setw(20) << left << (it->name) << setw(20) << (it->data) << endl;
		cout << endl;
		return true;
	}
	else
...
template <class T>
bool AssocArray<T>::AddItem(string name, T data)
{
...
template <class T>
bool AssocArray<T>::DelItem(string name) 
{
...
template <class T>
T AssocArray<T>::GetValue(string name) 
{
	cout << endl << setw(20) << left << "KEY" << setw(20) << "VALUE" << endl << endl;
	typename list<Data>::iterator it=Key(name);
	if(it!=array.end())
...
template <class T>
bool AssocArray<T>::FindKey(T data) 
{
	int state=0;
